...
class DualArmEnv(MujocoEnv, utils.EzPickle):
    def __init__(self, **kwargs):
        ...
        self.observation_space = spaces.Box(
            low=-np.inf,
            high=np.inf,
            shape=(2, 10 + 3 + 1),  # 2 agents, 5 joint positions, 5 joint velocities, ball position, ball velocity, thrower indicator
            dtype=np.float32
        )
    ...
    def step(self, action):
        ...
        # Update timer
        self._time_since_last_switch += self.dt * self.frame_skip
    ...
    def _get_obs(self):
        # Each agent gets: 5 joint pos, 5 joint vel, 3 ball pos, 1 thrower indicator
        ...
        # For each agent, stack their own joint pos/vel, ball pos, thrower indicator
        ...
        # Stack as (2, 14) for (n_agents, obs_dim)
    ...
    def _is_done(self):
        ...
        return (ball_pos[2] < 0.01 or  # Ball too low
                abs(ball_pos[0]) > 3.0 or  # Ball too far left/right
                abs(ball_pos[0]) < 0.5)
